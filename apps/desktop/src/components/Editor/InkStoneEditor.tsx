...
export function InkStoneEditor({ docId, isActive = true, onContentChange }: InkEditorProps) {
  ...
  const { fontSize, lineHeight, spellCheck, codeBlockShowLineNumbers, codeBlockEnableWordWrap } =
    usePreferencesStore();
  ...
  useEffect(() => {
    const editor = editorRef.current;
    console.log('[InkStoneEditor] Mode effect triggered:', {
      editorMode,
    ...
    const handleModeChange = async () => {
      if (editorMode === 'source') {
        ...
        try {
          ...
          console.log(
            '[InkStoneEditor] Switching back to preview mode, content changed:',
            contentChanged,
          );
          ...
          if (sourceContent?.current && contentChanged) {
            ...
            console.log(
              '[InkStoneEditor] Importing modified markdown, length:',
              sourceContent.current.length,
            );
    ...
    try {
      ...
      const registerHost = () => {
        if (editor.host) {
          ...
          const { codeBlockShowLineNumbers, codeBlockEnableWordWrap } =

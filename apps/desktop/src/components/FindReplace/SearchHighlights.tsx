...
import { type MatchResult, useFindReplaceStore } from '../../stores/findReplace';
import { clearTextNodeCache, getBlockTextInfo } from '../../utils/textNodeCache';
...
function clearAllHighlights(
  highlightRef?: React.MutableRefObject<Highlight | null>,
  currentHighlightRef?: React.MutableRefObject<Highlight | null>,
) {
  if (supportsHighlightAPI) {
    ...
    const editorHost =
      document.querySelector('editor-host') || document.querySelector('.ink-page-viewport');
...
export function SearchHighlights({ docId: _docId }: SearchHighlightsProps) {
  ...
  const isMatchVisible = useCallback((match: MatchResult, containerRect: DOMRect): boolean => {
    ...
    const buffer = 200;
    return (
      lineRect.bottom >= containerRect.top - buffer && lineRect.top <= containerRect.bottom + buffer
    );
  }, []);
  ...
  useEffect(() => {
    ...
    const scrollContainer =
      document.querySelector('.ink-page-viewport') || document.querySelector('editor-host');
  ...
  useEffect(() => {
    console.log(
      '[SearchHighlights] isOpen changed:',
      isOpen,
    ...
    if (!isOpen) {
      ...
      console.log(
        '[SearchHighlights] After delete, CSS.highlights has:',
        supportsHighlightAPI ? Array.from(CSS.highlights.keys()) : 'N/A',
      );
      clearTextNodeCache();

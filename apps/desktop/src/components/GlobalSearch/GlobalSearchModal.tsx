      ...
      <div className="search-modal" role="dialog" aria-modal="true">
        <div className="search-input-container">
          ...
          <input
            ...
            placeholder={
              rootPath ? t('search.placeholderWithFolder') : t('search.placeholderNoFolder')
            }
            value={query}
            ...
            results.map((result, index) => (
              ...
              >
                <div className="search-result-header">
                  ...
                  {result.isNameMatch && (
                    <span className="search-name-badge">{t('search.badge.nameMatch')}</span>
                  )}
                </div>
                {result.matches && result.matches.length > 0 && (
                  <div className="search-result-matches">
                    {result.matches.slice(0, 3).map((match, matchIndex) => {
                      ...
                      const { text, offset } = truncateLine(
                        match.lineContent,
                        match.matchStart || 0,
                      );

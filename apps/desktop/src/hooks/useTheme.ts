  ...
  const applyThemeCSS = useCallback(
    async (themeId: string) => {
      ...
      try {
        ...
        const cachedData = cssCache.get(themeId);
        ...
        if (!isBuiltinTheme) {
          ...
          let baseCSS: string;
          const baseCachedData = cssCache.get(baseThemeId);
          ...
          const processedBaseCSS = baseCSS.replace(
            /:root(\[data-theme=["'][^"']*["']\])?\s*\{/g,
            ':root:root {',
          );
        ...
        setTimeout(() => {
          const bgColor = getComputedStyle(document.documentElement)
            .getPropertyValue('--ink-background-primary-color')
            .trim();
          console.log('[Theme] --ink-background-primary-color:', bgColor || '(empty)');

          const toolbarShadow = getComputedStyle(document.documentElement)
            .getPropertyValue('--ink-toolbar-shadow')
            .trim();

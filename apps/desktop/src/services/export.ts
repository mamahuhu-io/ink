 ...
 */
async function loadImageData(blocks: BlockData[], embedImages: boolean): Promise<void> {
 ...
 */
export async function exportToHtml(filePath: string, store: any): Promise<void> {
  if (!isTauri()) {
 ...
 */
async function exportToHtmlFallback(filePath: string, store: any): Promise<void> {
  const root = store?.root;
  if (!root) {
    throw new Error('Document is empty or not loaded');
  }
 ...
 */
export async function exportToDocx(filePath: string, store: any): Promise<void> {
  if (!isTauri()) {
  ...
  try {
    ...
    function extractTextContent(textContent: any): {
      text: string;
      ...
      switch (flavour) {
        ...
        case 'ink:list': {
          ...
          if (listType === 'todo') {
            ...
            paragraphs.push(
              new Paragraph({
                children: [new TextRun({ text: checkMark }), ...createTextRuns(textContent, links)],
                bullet: { level: 0 },
              }),
            );
 ...
 */
export async function exportToImage(filePath: string): Promise<void> {
  if (!isTauri()) {
  ...
  try {
    ...
    if (!pageRoot) {
      ...
      pageRoot =
        editorContainer.querySelector('ink-page-root') || document.querySelector('ink-page-root');
    }

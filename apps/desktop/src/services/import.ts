...
import { createTransformerContext, DOC_INIT_DELAY } from './transformerUtils';
 ...
 */
async function importMarkdown(filePath: string, content: string, docId: string): Promise<void> {
 ...
 * Import DOCX file
 */
async function importDocx(filePath: string, fileData: Uint8Array, docId: string): Promise<void> {
 ...
 * Import HTML file
 */
async function importHtml(filePath: string, content: string, docId: string): Promise<void> {
  const { getOrCreateDoc } = await import('../stores/editor');

...
import { listDirectory, pickFolder, deletePath, renamePath, writeFile, createDirectory, type FileEntry } from '../services'
    ...
    (set, get) => ({
      ...
      refresh: async () => {
        ...
        try {
          ...
          const loadExpandedRecursive = async (
            nodes: FileTreeNode[]
          ): Promise<FileTreeNode[]> => {
            return Promise.all(
              nodes.map(async (node) => {
      ...
      revealFile: async (filePath: string) => {
        const { rootPath, setRootPath, loadDirectory, selectFile } = get()

...
import { findLineByOffset, getBlockTextInfo } from '../utils/textNodeCache';
import { getOrCreateDoc } from './editor';
...
export const useFindReplaceStore = create<FindReplaceStore>()((set, get) => ({
  ...
  replaceCurrent: (docId: string) => {
    const { matches, currentMatchIndex, replaceQuery, searchQuery, caseSensitive, useRegex } =
      get();
    if (matches.length === 0) return;

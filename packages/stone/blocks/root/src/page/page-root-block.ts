...
export class PageRootBlockComponent extends BlockComponent<RootBlockModel> {
  static override styles = css`
    editor-host:has(> ink-page-root, * > ink-page-root) {
      display: block;
      height: 100%;
    }
    ...
    .ink-page-root-block-container {
      ...
      font-weight: 400;
      max-width: var(--ink-editor-width);
      margin: 0 auto;
      ...
      /* prettier-ignore */
      padding-bottom: var(--ink-editor-bottom-padding, ${DOC_BOTTOM_PADDING}px);
    }
  ...
  private _createDefaultNoteBlock() {
    ...
    const noteId = store.addBlock('ink:note', {}, store.root?.id);
  ...
  override connectedCallback() {
    ...
    this.bindHotKey({
      ...
      ArrowUp: () => {
        ...
        if (!prevNote || prevNote.flavour !== 'ink:note') {
        ...
        const notes = this.store.getModelsByFlavour('ink:note');
    ...
    this.handleEvent('click', ctx => {
      ...
      if (!isClickOnBlankArea && !this.store.readonly$.value) {
        ...
        if (
          ...
          focusTextModel(this.std, lastBlock.id);
        }
        return;
      }

  ...
  implements MenuComponentInterface
{
  ...
  renderSearch() {
    ...
    return html` <div style=${searchStyle} class="ink-menu-search-container">
      <div
        style="font-size:20px;display:flex;align-items:center;color: var(--ink-text-secondary-color)"
      >
        ${SearchIcon()}
      </div>
      <input
        autocomplete="off"
        class="ink-menu-search"
    ...
    return html`
      ...
      >
        ${title.onBack
          ...
          : nothing}
        <div
          style="flex:1;font-weight:500;font-size: 14px;line-height: 22px;color: var(--ink-text-primary-color)"
        >
          ${title.text}
        </div>
        ${title.postfix?.()}

...
export const datePickerStyle = css`
  ...
  .popup.date-picker {
    background: var(--ink-background-overlay-panel-color);
    border-radius: 12px;
    box-shadow: var(--ink-menu-shadow);
  }
  ...
  .date-picker-small-action:hover {
    color: var(--ink-icon-hover-color);
    background: var(--ink-icon-hover-background);
  }
  ...
  .interactive::before {
    opacity: 0;
    transition: none;
    box-shadow: 0 0 0 3px var(--ink-primary-color);
  }
  ...
  .interactive:focus-visible {
    outline: none;
    outline: 1px solid var(--ink-primary-color);
  }

...
/**
 * Similar to `<ink-portal>`, but only renders once when called.
 *
...
export function createSimplePortal({
  ...
  if (identifyWrapper) {
    portalRoot.classList.add('ink-portal');
  }
  if (shadowDom) {
  ...
  // ink's modal will set pointer-events: none to body
  // in order to avoid the issue that the floating element in stone cannot be clicked

      ...
      this.__dispose = effect(() => {
        if (updateFromLit) {
          ...
          try {
            ...
            // When DOM is manually manipulated (e.g., replaceChildren() + delete _$litPart$),
            // Lit may throw "ChildPart has no parentNode" error.
            // In this case, we request a new update cycle after the DOM state stabilizes.
            if (
              e instanceof Error &&
              e.message.includes('ChildPart') &&

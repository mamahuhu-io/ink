import { IS_IPAD } from '@ink/stone-global/env';
import { Vec } from '@ink/stone-global/gfx';
...
class DragController extends PointerControllerBase {
  ...
  private readonly _move = (event: PointerEvent) => {
    ...
    // buttons is a bitmask: 1 = primary button (left), 2 = secondary (right), 4 = auxiliary (middle)
    // If buttons is 0, no button is pressed - this can happen if pointerup was missed
  ...
  private readonly _onVisibilityChange = () => {
    if (document.hidden && this._dragging) {
      this._reset();
    }
  };

  // Handle window blur (switch to another app)
  ...
  listen() {
    ...
    // Add visibility and focus change handlers to reset drag state
    disposables.addFromEvent(document, 'visibilitychange', this._onVisibilityChange);
    disposables.addFromEvent(window, 'blur', this._onWindowBlur);

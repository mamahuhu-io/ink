...
export const InlineSpecIdentifier = createIdentifier<unknown>('InkInlineSpec');
...
export function InlineSpecExtension<TextAttributes extends BaseTextAttributes>(
  ...
  if (typeof nameOrSpec === 'string') {
    const identifier = InlineSpecIdentifier<InlineSpecs<TextAttributes>>(nameOrSpec);
    return {
      identifier,
      setup: (di) => {
  ...
  const identifier = InlineSpecIdentifier<InlineSpecs<TextAttributes>>(nameOrSpec.name as string);
  return {
    identifier,
    setup: (di) => {

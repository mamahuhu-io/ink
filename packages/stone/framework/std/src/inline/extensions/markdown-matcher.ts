...
export const MarkdownMatcherIdentifier = createIdentifier<unknown>('InkMarkdownMatcher');

export function InlineMarkdownExtension<TextAttributes extends BaseTextAttributes>(
  matcher: InlineMarkdownMatch<TextAttributes>,
): ExtensionType & {
  identifier: ServiceIdentifier<InlineMarkdownMatch<TextAttributes>>;
} {
  const identifier = MarkdownMatcherIdentifier<InlineMarkdownMatch<TextAttributes>>(matcher.name);

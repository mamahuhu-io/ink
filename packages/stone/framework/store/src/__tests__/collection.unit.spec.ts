...
describe('basic', () => {
  ...
  it('doc ready lifecycle', () => {
    ...
    store.load(() => {
      ...
      store.addBlock('ink:note', {}, rootId);
    });
...
describe('addBlock', () => {
  ...
  it('can add model with props', () => {
    const doc = createTestDoc();
    doc.addBlock('ink:page', { title: new Text('hello') });
  ...
  it('can add multi models', () => {
    ...
    assert.deepEqual(serializCollection(doc.rootDoc).spaces[spaceId].blocks, {
      ...
      '1': {
        'sys:children': ['2', '3', '4'],
        'sys:flavour': 'ink:note',
        'sys:id': '1',
        'sys:version': 1,
      },
      '2': {
        'sys:children': [],
        'sys:flavour': 'ink:paragraph',
        'sys:id': '2',
        'prop:text': '',
        'prop:type': 'text',
        'sys:version': 1,
  ...
  it('can observe slot events', async () => {
    ...
    assert.equal(block.flavour, 'ink:page');
  });
  ...
  it('can add block to root', async () => {
    ...
    queueMicrotask(() => {
      const rootId = doc.addBlock('ink:page');
      noteId = doc.addBlock('ink:note', {}, rootId);
    });
    await waitOnce(doc.slots.rootAdded);
    const { root } = doc;
    if (!root) throw new Error('root is null');

    assert.equal(root.flavour, 'ink:page');

    doc.addBlock('ink:paragraph', {}, noteId!);
    assert.equal(root.children[0].flavour, 'ink:note');
...
describe('deleteBlock', () => {
  ...
  it('bring children to parent', () => {
    ...
    const rootId = doc.addBlock('ink:page', {});
    const noteId = doc.addBlock('ink:note', {}, rootId);
    const p1 = doc.addBlock('ink:paragraph', {}, noteId);
    doc.addBlock('ink:paragraph', {}, p1);
    doc.addBlock('ink:paragraph', {}, p1);
  ...
  it('bring children to other block', () => {
    ...
    const noteId = doc.addBlock('ink:note', {}, rootId);
    const p1 = doc.addBlock('ink:paragraph', {}, noteId);
    const p2 = doc.addBlock('ink:paragraph', {}, noteId);
    doc.addBlock('ink:paragraph', {}, p1);
    doc.addBlock('ink:paragraph', {}, p1);
    doc.addBlock('ink:paragraph', {}, p2);
  ...
  it('can delete model with parent', () => {
    ...
    const noteId = doc.addBlock('ink:note', {}, rootModel.id);
...
describe('getBlock', () => {
  it('can get block by id', () => {
    ...
    assert.equal(text.flavour, 'ink:paragraph');

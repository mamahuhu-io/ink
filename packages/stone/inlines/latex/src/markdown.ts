...
export const LatexExtension = InlineMarkdownExtension<InkTextAttributes>({
  name: 'latex',
    ...
    if (blockPrefix === '$$$$') {
      ...
      blockComponent.host.updateComplete
        .then(() => {
          ...
          if (!latexBlock || latexBlock.flavour !== 'ink:latex') return;
    ...
    if (inlinePrefix === '$$') {
      ...
      inlineEditor
        ...
        .then(async () => {
          ...
          const latexNode = text.parentElement?.closest('ink-latex-node');
          if (!latexNode) return;

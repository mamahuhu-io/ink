...
/**
 * Set a custom i18n getter function for link popup translations
 */
export function setLinkPopupI18nGetter(getter: (key: string, fallback: string) => string) {
  i18nGetter = getter;
}
...
export class LinkPopup extends WithDisposable(ShadowlessElement) {
  ...
  private readonly _createTemplate = () => {
    ...
    return html`
      <div class="ink-link-popover create">
        <input
          id="link-input"
          class="ink-link-popover-input"
          type="text"
          spellcheck="false"
  ...
  private readonly _editTemplate = () => {
    ...
    return html`
      <div class="ink-link-edit-popover">
        <div class="ink-edit-area text">
          <input
            class="ink-edit-input"
            id="text-input"
            type="text"
            placeholder="${t('enterText', 'Enter text')}"
            @input=${this._updateConfirmBtn}
          />
          <label class="ink-edit-label" for="text-input">${t('text', 'Text')}</label>
        </div>
        <div class="ink-edit-area link">
          <input
            id="link-input"
            class="ink-edit-input"
            type="text"
            ...
            @input=${this._updateConfirmBtn}
          />
          <label class="ink-edit-label" for="link-input">${t('link', 'Link')}</label>
        </div>
        ${this._confirmBtnTemplate()}
      </div>
    `;
  ...
  @query('.ink-confirm-button')
  accessor confirmButton: EditorIconButton | null = null;

  @property({ attribute: false })
  accessor inlineEditor!: InkInlineEditor;

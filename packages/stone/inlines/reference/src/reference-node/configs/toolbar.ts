...
import { InkReference } from '../reference-node';
    ...
    },
    {
      ...
      actions: [
        ...
        },
        {
          ...
          run(ctx) {
            const target = ctx.message$.peek()?.element;
            if (!(target instanceof InkReference)) return;
        ...
        },
        {
          ...
          disabled(ctx) {
            ...
            if (target.closest('ink-embed-synced-doc-block')) return true;
      ...
      when(ctx) {
        const target = ctx.message$.peek()?.element;
        if (!(target instanceof InkReference)) return false;

...
import { BaseBlockTransformer } from '@ink/stone-store';
...
export class AttachmentBlockTransformer extends BaseBlockTransformer<AttachmentBlockProps> {
  override async fromSnapshot(
    ...
    if (!payload.assets.isEmpty() && sourceId) await payload.assets.writeToBlob(sourceId);

    return snapshotRet;
  }

  override toSnapshot(snapshot: ToSnapshotPayload<AttachmentBlockProps>): BlockSnapshotLeaf {

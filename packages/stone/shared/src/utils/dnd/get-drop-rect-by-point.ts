...
export function getDropRectByPoint(
  ...
  if (isDatabase) {
    ...
    for (; i < len; i++) {
      ...
      if (e.classList.contains('ink-database-block-row-cell-content')) {
      ...
      if (e.classList.contains('ink-database-block-row')) {
  ...
  if (parent?.classList.contains('ink-database-block-row-cell-content')) {
    result.flag = DropFlags.Database;
...
function getDatabaseBlockTableElement(element: Element) {
  return element.querySelector('.ink-database-block-table');
}
...
function getDatabaseBlockColumnHeaderElement(element: Element) {
  return element.querySelector('.ink-database-column-header');
}
...
function getCellRect(element: Element, bounds?: DOMRect) {
  if (!bounds) {
    const table = element.closest('.ink-database-block-table');
    if (!table) {
      return null;
    }
    bounds = table.getBoundingClientRect();
  }
  // ink-database-block-row-cell
  const col = element.parentElement;
  if (!col) {
    return null;
  }
  // ink-database-block-row

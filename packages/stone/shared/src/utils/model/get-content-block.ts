...
// import { FrameBlockModel } from '@ink/stone-model';
...
// [REMOVED] matchModels no longer needed without FrameBlockModel
...
export function getPrevContentBlock(
  ...
  const getPrev = (model: BlockModel) => {
    ...
    if (index > 0) {
      ...
      if (parent.role === 'root' && model.role === 'hub') {
        while (prev && prev.flavour !== 'ink:note') {
  ...
  const iterate: (model: BlockModel) => BlockModel | null = (
    ...
    if (prev) {
      // [REMOVED] FrameBlockModel check - Edgeless feature not needed
 ...
 * ```
 *
 * NOTE: this method will skip the `ink:note` block
 */

...
const pureLightTheme = {
  ...
  aiEmptyShadow:
    "0px 4px 4px rgba(66, 65, 73, 0.10), 0px 6px 8px rgba(66, 65, 73, 0.05)",
...
export {
  darkThemeVariables as darkThemeV2,
  lightThemeVariables as lightThemeV2,
};
...
export function cssVar(
  ...
  const varName = key.startsWith("--")
    ? key
    : themeToVar(key as keyof InkTheme);
...
function createCssVarProxy(prefix: string = ""): any {
  return new Proxy(_cssVarV2, {
    get(_, prop) {
      ...
      if (
        typeof lightThemeVariables[newPrefix as ThemeVariableKey] !== "string"
      ) {
    ...
    apply(_, __, [varName, fallback]) {
      return prefix
        ? _cssVarV2(prefix as ThemeVariableKey, varName)

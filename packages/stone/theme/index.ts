...
// ==================== V1 Theme (Legacy) ====================
// Used for old components, converts camelCase to kebab-case
...
export const themeToVar = (themeName: keyof InkTheme) =>
  `--ink-${camelToKebab(themeName)}` as keyof InkCssVariables;

const objectEntries = <T extends Record<string, any>>(obj: T): [keyof T, T[keyof T]][] =>
...
export type InkTheme = typeof lightTheme & {
  editorMode: 'page' | 'edgeless';
};

export type InkCssVariables = {
  [Key in `--ink-${Kebab<keyof InkTheme>}`]: string;
};
...
const pureLightTheme = {
  ...
  overlayPanelShadow: '0px 1px 6px rgba(0,0,0,0.16), 0px 8px 14px rgba(0,0,0,0.08)',
  ...
  overlayShadow: '0px 1px 6px 0px rgba(0, 0, 0, 0.16), 0px 8px 14px 0px rgba(0, 0, 0, 0.08)',
  popoverShadow:
  ...
  noteShadowBox: '0px 0px 4px 0px rgba(0, 0, 0, 0.14), 0px 2px 12px 0px rgba(0, 0, 0, 0.14)',
  noteShadowSticker:
  ...
  noteShadowFloat: '0px 0px 32px 0px rgba(0, 0, 0, 0.09), 0px 0px 3px 0px rgba(0, 0, 0, 0.10)',
...
const pureDarkTheme = {
  ...
  overlayPanelShadow: '0px 1px 6px rgba(0,0,0,0.2), 0px 8px 14px rgba(0,0,0,0.3)',
  ...
  overlayShadow: '0px 1px 6px 0px rgba(0, 0, 0, 0.72), 0px 8px 14px 0px rgba(0, 0, 0, 0.32)',
  popoverShadow:
    '0px 0px 30px 0px rgba(0, 0, 0, 0.1), 0px 0px 8px 0px rgba(0, 0, 0, 0.18), 0px 0px 0px 0.5px rgba(48, 48, 48, 1) inset',
  floatButtonShadow: '0px 0px 10px rgba(0, 0, 0, 0.05), 0px 0px 0px 0.5px rgba(0, 0, 0, 0.1)',
  floatButtonShadow1: '0px 4px 6px -3px rgba(0, 0, 0, 0.1), 0px 10px 12px -2px rgba(0, 0, 0, 0.1)',
  floatButtonShadow2:
  ...
  noteShadowBox: '0px 0px 2px 0px rgba(255, 255, 255, 0.44), 0px 2px 12px 0px rgba(0, 0, 0, 0.30)',
  noteShadowSticker:
...
const createLegacyVariables = (theme: Partial<InkTheme>) => {
  return objectEntries(theme).reduce((variables, [key, value]) => {
    return { ...variables, [themeToVar(key)]: value };
  }, {} as InkCssVariables);
};

export const lightCssVariables = createLegacyVariables(lightTheme);
export const darkCssVariables = createLegacyVariables(darkTheme);
export const printCssVariables = createLegacyVariables(printTheme);

// ==================== Theme Variables (Figma Design) ====================
// Primary theme system with path-based keys like 'text/primary'

type NestedTheme = typeof nestedLightTheme | typeof nestedDarkTheme;

// Re-export for compatibility
export { darkThemeVariables, lightThemeVariables };
export type ThemeVariables = typeof lightThemeVariables;
export type ThemeVariableKey = keyof ThemeVariables;

// Legacy exports for compatibility
export { darkThemeVariables as darkThemeV2, lightThemeVariables as lightThemeV2 };
export type InkThemeV2 = ThemeVariables;
export type InkThemeKeyV2 = ThemeVariableKey;

/**
 * Convert theme variable key to CSS variable name
 * @example themeVarToVar('text/primary') => '--ink-text-primary'
 */
export function themeVarToVar(theme: ThemeVariableKey) {
  const parts = theme.split('/');
  return `--ink-${parts.join('-')}`;
}

function createThemeVariables(theme: ThemeVariables) {
  const vars: Record<string, string> = {};
  objectKeys(theme).forEach((key) => {
    vars[themeVarToVar(key)] = theme[key];
  });
  return vars;
}

export const lightThemeCssVariables = createThemeVariables(lightThemeVariables);
export const darkThemeCssVariables = createThemeVariables(darkThemeVariables);

// Legacy exports
export { lightThemeCssVariables as lightCssVariablesV2 };
export { darkThemeCssVariables as darkCssVariablesV2 };

// Combined CSS variables (legacy + theme variables)
...
export const scopedLightCssVariables = {
  ...createLegacyVariables(pureLightTheme),
  ...lightThemeCssVariables,
};
export const scopedDarkCssVariables = {
  ...createLegacyVariables(pureDarkTheme),
  ...darkThemeCssVariables,
};

/**
 * Get INK css variable name type safely (legacy camelCase format)
 * @param key in camel or kebab format
 * @param fallback
 * @returns
 */
export function cssVar(key: keyof InkCssVariables | keyof InkTheme, fallback?: string) {
  const varName = key.startsWith('--') ? key : themeToVar(key as keyof InkTheme);
  return `var(${varName}${fallback ? `, ${fallback}` : ''})`;
}

// ==================== cssVarV2: Theme Variable Access ====================

function _cssVarV2(key: ThemeVariableKey, fallback?: string) {
  return `var(${themeVarToVar(key)}${fallback ? `, ${fallback}` : ''})`;
}
...
function createCssVarProxy(prefix: string = ''): any {
  return new Proxy(_cssVarV2, {
    get(_, prop) {
      ...
      if (typeof lightThemeVariables[newPrefix as ThemeVariableKey] !== 'string') {
        return createCssVarProxy(newPrefix);
      }

      // found the value
      return _cssVarV2(newPrefix as ThemeVariableKey);
    },
    apply(_, __, [varName, fallback]) {
      return prefix ? _cssVarV2(prefix as ThemeVariableKey, varName) : _cssVarV2(varName, fallback);
    },
  });
}

/**
 * Get INK css variable name type safely
 * @param key as copied from Figma design. __e.g. `text/primary`__
 *
 * ```ts
 * import { cssVarV2 } from 'stone/theme';
 *

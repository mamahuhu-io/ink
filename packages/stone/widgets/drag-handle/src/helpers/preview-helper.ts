...
import { BlockStdScope, BlockViewIdentifier } from '@ink/stone-std';
import type {
  BlockModel,
  BlockViewType,
  ExtensionType,
...
export class PreviewHelper {
  ...
  getPreviewStd = (blockIds: string[]) => {
    ...
    const extensions = [
      ...
      {
        setup(di) {
          di.override(BlockViewIdentifier('ink:image'), () => {
            return (model: BlockModel) => {
              const parent = model.store.getParent(model.id);

              if (parent?.flavour === 'ink:surface') {
                return literal`ink-edgeless-placeholder-preview-image`;
              }

              return literal`ink-placeholder-preview-image`;
            };
          });
        },
      } as ExtensionType,
    ...
    const noteBlock = this.widget.host.querySelector('ink-note');

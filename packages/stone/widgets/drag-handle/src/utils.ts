...
export const insideDatabaseTable = (element: Element) => {
  return !!element.closest('.ink-database-block-table');
};
...
export const getParentNoteBlock = (blockComponent: BlockComponent) => {
  return blockComponent.closest('ink-note, ink-edgeless-note') ?? null;
};
...
export const getClosestBlockByPoint = (
  ...
  const blockSelector =
    '.ink-note-block-container > .ink-block-children-container > [data-block-id]';

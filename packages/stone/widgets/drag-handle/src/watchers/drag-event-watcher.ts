...
const EMBED_IFRAME_DEFAULT_WIDTH_IN_SURFACE = 752;
...
import { surfaceRefToEmbed } from '@ink/stone-shared/adapters';
import {
...
export type DragBlockPayload = DragPayload<DragBlockEntity, DragFromInkStone>;

declare module '@ink/stone-std' {
  interface DNDEntity {
...
export class DragEventWatcher {
    ...
    const rewrite = (block: BlockSnapshot) => {
      if (block.flavour === 'ink:surface') {
        ...
        if (block.flavour === 'ink:embed-iframe') {
          ...
          block.flavour === 'ink:attachment' ||
          block.flavour === 'ink:bookmark' ||
          block.flavour.startsWith('ink:embed-')
        ) {
  ...
  private readonly _dropAsGfxBlock = (
    ...
    if (!groupByParent.empty?.length) {
      ...
      if (groupByParent['ink:surface']) {
        const content = groupByParent['ink:surface'];
      ...
      if (groupByParent['ink:page']) {
        const content = groupByParent['ink:page'];
        const pageSnapshot = {
          ...snapshot,
          content,
        };
  ...
  private async _dropToModel(
    ...
    try {
      ...
      if (snapshot.content.length === 1) {
        const [first] = snapshot.content;
        if (first.flavour === 'ink:embed-linked-doc') {
  ...
  private _makeDropTarget(view: BlockComponent) {
    ...
    if (
      // ink:surface block can't be drop target in any modes
    ...
    if (matchModels(view.model, [AttachmentBlockModel])) {

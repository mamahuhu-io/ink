...
import {
  INK_SLASH_MENU_MAX_HEIGHT,
  INK_SLASH_MENU_TOOLTIP_TIMEOUT,
  INK_SLASH_MENU_TRIGGER_KEY,
} from './consts.js';
  ...
  override connectedCallback() {
    ...
    createKeydownObserver({
      ...
      interceptor: (event, next) => {
        ...
        if (key === INK_SLASH_MENU_TRIGGER_KEY) {
  ...
  override render() {
    const slashMenuStyles = this._position
      ? {
          ...
          maxHeight: `${Math.min(this._position.height, INK_SLASH_MENU_MAX_HEIGHT)}px`,
        }
      : {
          visibility: 'hidden',
        };
...
export class InnerSlashMenu extends WithDisposable(LitElement) {
  ...
  private readonly _openSubMenu = (item: SlashMenuSubMenu) => {
    ...
    subMenuElement.style.zIndex = `calc(var(--ink-z-index-popover) + ${this.depth})`;
  ...
  private readonly _renderActionItem = (item: SlashMenuActionItem) => {
    ...
    >
      ...
      html`<ink-tooltip
        ...
        .hoverOptions=${{
          enterDelay: INK_SLASH_MENU_TOOLTIP_TIMEOUT,
          allowMultiple: false,
        }}
      >

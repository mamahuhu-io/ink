...
import { INK_SLASH_MENU_TRIGGER_KEY } from './consts';
...
export class InkSlashMenuWidget extends WidgetComponent {
  ...
  private readonly _handleInput = (
    ...
    if (this.block?.model.flavour !== 'ink:page') {
  ...
  private readonly _onCompositionEnd = (ctx: UIEventStateContext) => {
    ...
    if (event.data !== INK_SLASH_MENU_TRIGGER_KEY) return;
  ...
  private readonly _onKeyDown = (ctx: UIEventStateContext) => {
    ...
    if (event.isComposing || key !== INK_SLASH_MENU_TRIGGER_KEY) return;

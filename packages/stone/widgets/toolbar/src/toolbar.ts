...
export const INK_TOOLBAR_WIDGET = 'ink-toolbar-widget';

export class InkToolbarWidget extends WidgetComponent {
  ...
  updateWithSurface(ctx: ToolbarContext, activated: boolean, elementIds: string[]) {
    ...
    if (activated && surface) {
      ...
      if (hasLocked) {
        flavour = 'ink:surface:locked';
      } else {
        if (paired.length === 1) {
          flavour = paired[0][0];
          if (flavour === 'ink:surface:shape' && paired[0][1].length === 1) {
      ...
      if (!sideOptions) {
        ...
        if (flavours.has('ink:surface:frame')) {
          sideOptions = sideMap.get('ink:surface:frame') ?? null;
        } else if (flavours.has('ink:surface:group')) {
          sideOptions = sideMap.get('ink:surface:group') ?? null;
        }
      }
    }

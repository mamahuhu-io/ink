...
export const sideMap = new Map([
  ...
  ['ink:surface:frame', { top: 28 }],
  // includes group element
  ['ink:surface:group', { top: 20 }],
  // has only one shape element
  ['ink:surface:shape', { top: 26, bottom: -26 }],
]);
...
export function renderToolbar(toolbar: EditorToolbar, context: ToolbarContext, flavour: string) {
  ...
  const actions = [
    flavour,
    `custom:${flavour}`,
    hasSurfaceScope ? ['ink:surface:*', 'custom:ink:surface:*'] : [],
    'ink:*',
    'custom:ink:*',
  ]
    .flat()
  ...
  // Avoids shaking
  if (flavour === 'ink:note' && context.std.range.value) {
